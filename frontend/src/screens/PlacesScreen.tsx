...
import { MapPin, Star, Search, X, TrendingUp, Award, Navigation, Wallet, WifiOff, SlidersHorizontal, Bookmark, LayoutGrid, List, Map as MapIcon, Shuffle, CheckCheck, FolderPlus, Plus, Clock } from 'lucide-react';
...
const MapView = ({ places, userCoords, onSelectPlace }: {
  places: Place[];
  ...
  const placesWithCoords = places.filter(p => (p.coordinates?.lat || p.lat) && (p.coordinates?.lng || p.lng));
...
const PlaceFilterPanel = ({
  ...
  onChange: (f: PlaceFilterState) => void;
}) => {
  ...
  const activeCount = [
    local.priceLevel > 0,
    local.season !== 'All',
    local.wheelchair || local.family || local.parking,
      ...
      <div className="relative bg-white w-full max-w-lg rounded-t-3xl shadow-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                { k: 'wheelchair', icon: '♿', l: 'Wheelchair' },
                { k: 'family', icon: '👨‍👩‍👧', l: 'Family' },
                { k: 'parking', icon: '🅿', l: 'Parking' },
              ].map(({ k, icon, l }) => (
                <button key={k}
                  onClick={() => setLocal(f => ({ ...f, [k]: !(f as any)[k] }))}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${(local as any)[k] ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-200 hover:border-emerald-400'}`}>
                  {icon} {l}
                </button>
              ))}
            </div>
...
export const PlacesScreen = ({ t, initialPlaceId, onPlaceOpened }: { t: any; initialPlaceId?: string; onPlaceOpened?: () => void }) => {
  ...
  const [savedIds, setSavedIds] = useState<Set<string>>(() => {
    try { return new Set<string>(JSON.parse(localStorage.getItem('tripo_saved_places') || '[]') as string[]); } catch { return new Set<string>(); }
  });

  // Visited places
  const [visitedIds, setVisitedIds] = useState<Set<string>>(() => {
    try { return new Set<string>(JSON.parse(localStorage.getItem('tripo_visited_places') || '[]') as string[]); } catch { return new Set<string>(); }
  });
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('tripo_recent_searches') || '[]') as string[]; } catch { return []; }
  });
  ...
  useEffect(() => {
    if (!selectedPlace) {
      try { setSavedIds(new Set<string>(JSON.parse(localStorage.getItem('tripo_saved_places') || '[]') as string[])); } catch {}
    }
  }, [selectedPlace]);
  ...
  const toggleVisited = (placeId: string, e: React.MouseEvent) => {
    ...
    const list: string[] = JSON.parse(localStorage.getItem('tripo_visited_places') || '[]');
    const updated = isNowVisited ? list.filter(id => id !== placeId) : [...list, placeId];
    localStorage.setItem('tripo_visited_places', JSON.stringify(updated));
    setVisitedIds(new Set<string>(updated));
  ...
  const openAddToTrip = (placeId: string, placeName: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const activeFilterCount = [
    placeFilters.priceLevel > 0,
    placeFilters.season !== 'All',
    placeFilters.wheelchair || placeFilters.family || placeFilters.parking,
  ].filter(Boolean).length;

  const getPriceLevel = (p: Place): number => {
  ...
  // Curated lists (only when no active filters)
  const curatedLists = useMemo(() => {
    if (!places.length) return [];
    const lists: { title: string; places: Place[] }[] = [];

    const hiddenGems = places
      .filter(p => (p.ratingSummary?.avgRating ?? p.rating ?? 0) >= 4.2 &&
        (p.ratingSummary?.reviewCount ?? 0) < 20 &&
        (p.photos?.[0] || p.image))
    ...
    const familyPlaces = places
      .filter(p => p.accessibility?.family && (p.photos?.[0] || p.image))
      .slice(0, 8);
    if (familyPlaces.length >= 3) lists.push({ title: '👨‍👩‍👧 Family Friendly', places: familyPlaces });

    const budgetPlaces = places.filter(p => {
      const pr = getPriceLevel(p);
      return (pr === 1 || pr === 2) && (p.photos?.[0] || p.image);
    }).slice(0, 8);
  ...
  const QUICK_FILTERS: { id: QuickFilter; label: string; icon: React.ReactNode }[] = [
    { id: 'open_now', label: t.filterOpenNow || 'Open Now', icon: <span className="w-1.5 h-1.5 bg-current rounded-full animate-pulse" /> },
  ...
  const renderGridCard = (place: Place) => {
    ...
    const isVisited = visitedIds.has(placeId);
    const duration = formatDuration(place.duration);
    const dist = quickFilter === 'near_me' && userCoords
    ...
    return (
      ...
      >
        {isVisited && (
          <div className="absolute inset-0 z-10 pointer-events-none rounded-2xl ring-2 ring-emerald-400" />
        )}
        <div className="h-36 bg-slate-200 relative overflow-hidden">
          {img ? (
            <img
              src={img}
              className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 ${isVisited ? 'brightness-75' : ''}`}
              alt={place.name}
              loading="lazy"
            />
          ) : (
          ...
          {isVisited && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="bg-emerald-500/90 rounded-full p-1.5">
          ...
          {/* Bookmark button */}
          <button
            onClick={e => toggleSaved(placeId, e)}
            className={`absolute bottom-2 right-2 w-6 h-6 rounded-full flex items-center justify-center transition-all ${isSaved ? 'bg-rose-500 shadow-md' : 'bg-black/40 hover:bg-black/60'}`}
          >
        ...
        <div className="p-3">
          ...
          <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-slate-50">
            <button
              onClick={e => toggleVisited(placeId, e)}
              className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold transition-all ${isVisited ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600'}`}
            >
              <CheckCheck className="w-2.5 h-2.5" />
              {isVisited ? 'Visited' : 'Mark Visited'}
            </button>
            <button
              onClick={e => openAddToTrip(placeId, place.name, e)}
              className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500 hover:bg-teal-50 hover:text-teal-600 transition-all ml-auto"
            >
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          <button onClick={e => toggleSaved(placeId, e)}
            className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${isSaved ? 'bg-rose-100 text-rose-500' : 'bg-slate-100 text-slate-400 hover:bg-rose-50 hover:text-rose-400'}`}>
            <Bookmark className={`w-3.5 h-3.5 ${isSaved ? 'fill-rose-500' : ''}`} />
          </button>
          <button onClick={e => openAddToTrip(placeId, place.name, e)}
            className="w-7 h-7 rounded-full bg-slate-100 text-slate-400 hover:bg-teal-50 hover:text-teal-600 flex items-center justify-center transition-all">
            <FolderPlus className="w-3.5 h-3.5" />
          </button>
          <button onClick={e => toggleVisited(placeId, e)}
            className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${isVisited ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400 hover:bg-emerald-50 hover:text-emerald-500'}`}>
    ...
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-5 pt-5 pb-3">
        ...
        <div className="flex gap-2 items-center mb-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={t.placesSearch || 'Search by name, city, or tag...'}
              value={search}
            ...
            {/* Recent searches (shown when focused, no active search) */}
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-slate-100 z-30 overflow-hidden">
                ...
                {recentSearches.map(s => (
                  <button key={s} onMouseDown={() => { setSearch(s); setSearchFocused(false); }}
                    ...
                    <button onMouseDown={e => { e.stopPropagation(); const updated = recentSearches.filter(r => r !== s); setRecentSearches(updated); localStorage.setItem('tripo_recent_searches', JSON.stringify(updated)); }}
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            {CITY_LIST.map(city => (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-semibold transition-all border ${
                  cityFilter === city
      ...
      {viewMode === 'map' && (
        <div className="flex-1 overflow-hidden">
          <MapView places={filtered} userCoords={userCoords} onSelectPlace={p => setSelectedPlace(p)} />
        </div>
      )}
        ...
        <>
          ...
          {hasNetworkError && !isLoading && (
            <div className="mx-4 mt-3 flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
              ...
              <button
                onClick={() => {
                  setHasNetworkError(false); setIsLoading(true);
                  placeAPI.getPlaces()
          ...
          <div className="flex-1 overflow-y-auto p-4 pb-24">
            {isLoading ? (
              ...
              <>
                ...
                {/* Curated lists (shown when no active filters) */}
